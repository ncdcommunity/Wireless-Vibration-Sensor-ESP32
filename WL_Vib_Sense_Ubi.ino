...
#define TOKEN "BBFF-DAm8v1Nt9m3Ot7wNMfHBzrFLrIssSD" // Your Ubidots TOKEN
#define MQTT_CLIENT_NAME "ClientVBShightime123" 
#define VARIABLE_LABEL_TEMPC "tempC" // Assing the variable label
#define VARIABLE_LABEL_RMSX "rmsx"
#define VARIABLE_LABEL_RMSY "rmsy"
#define VARIABLE_LABEL_RMSZ "rmsz"
#define VARIABLE_LABEL_MINX "minx" // Assing the variable label
#define VARIABLE_LABEL_MINY "miny" // Assing the variable label
#define VARIABLE_LABEL_MINZ "minz" // Assing the variable label

#define DEVICE_LABEL "espVib" // Assig the device label
...
// 
  ...
  sprintf(payload, "%s", ""); // Cleans the payload
  sprintf(payload, "{\"%s\":", VARIABLE_LABEL_RMSX); // Adds the value
  sprintf(payload, "%s{\"value\":%s}", payload, String(rms_x).c_str()); // Adds the value
  sprintf(payload, "%s}", payload); // Closes the dictionary brackets
  Serial.println(payload);
  ...
  sprintf(payload, "%s", ""); // Cleans the payload
  sprintf(payload, "{\"%s\":", VARIABLE_LABEL_RMSY); // Adds the value
  sprintf(payload, "%s{\"value\":%s}", payload, String(rms_y).c_str()); // Adds the value
  sprintf(payload, "%s}", payload); // Closes the dictionary brackets
  Serial.println(payload);
  ...
  sprintf(payload, "%s", ""); // Cleans the payload
  sprintf(payload, "{\"%s\":", VARIABLE_LABEL_RMSZ); // Adds the value
  sprintf(payload, "%s{\"value\":%s}", payload, String(rms_z).c_str()); // Adds the value
  sprintf(payload, "%s}", payload); // Closes the dictionary brackets
  Serial.println(payload);
  Serial.println(client.publish(topic4,payload) ? "published" : "not published");
...
void reconnectMQTT() {
    // Loop until we're reconnected
     //String response=FPSTR(topic_name);
